...
# =============================================
def train_one_model(
    model, train_loader, val_loader, cfg: dict, epochs=80, device="cpu"
):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            ...
            print(
                f"  Ep {epoch + 1:3d}/{epochs} | tLoss={train_loss:.3f} vLoss={val_loss:.3f}"
...
def check_overfitting(histories: dict):
    ...
    for name, hist in histories.items():
        ...
        gap = vl - tl
        status = (
            "✓ Tốt" if gap < 0.15 else ("~ Chấp nhận" if gap < 0.30 else "✗ Overfit")
        )
...
def plot_training_curves(histories, save_dir):
    ...
    fig.suptitle(
        "Training Curves — RNN vs LSTM vs BiLSTM\n"
...
def plot_metrics_comparison(all_metrics, save_dir):
    ...
    ax.set_ylim(0, 1.12)
    ax.set_title(
        f"🏆 So sánh 3 mô hình — Best: {names[best_idx]}",
        fontsize=14,
        fontweight="bold",
    )
...
def plot_confusion_matrix(model, loader, device, save_dir):
    ...
    ax.set_title(
        "🎯 Confusion Matrix — BiLSTM (Model Tốt Nhất)", fontsize=14, fontweight="bold"
    )
    plt.tight_layout()
...
def plot_radar(all_metrics, save_dir):
    ...
    ax.set_title(
        "Radar Chart — Performance Comparison", fontsize=12, fontweight="bold", pad=20
    )
...
if __name__ == "__main__":
    ...
    # ── Sinh event-level CSV (format thầy yêu cầu — Bước 1) ──
    try:
        from module1_behavior.generate_data import generate_dataset
        generate_dataset(n_users=504, output_path="data/data_user500.csv")
        print("📄 Event-level logs → data/data_user500.csv (504 users)")
    except Exception as e:
        print(f"⚠️  Không sinh được event-level logs: {e}")

    # ── Tạo raw data cho training (Bước 2 — session features) ──
    ...
    val_ds = BehaviorDataset(
        X_val, y_val, scaler=base_train_ds.scaler, fit_scaler=False
    )
    test_ds = BehaviorDataset(
        X_test, y_test, scaler=base_train_ds.scaler, fit_scaler=False
    )

    base_loader = DataLoader(
        base_train_ds, batch_size=32, shuffle=True, pin_memory=(device == "cuda")
    )
    aug_loader = DataLoader(
        aug_train_ds, batch_size=32, shuffle=True, pin_memory=(device == "cuda")
    )
    ...
    for name, m in models_def.items():
        ...
        cfg = MODEL_CFG[name]
        print(
            f"  {name:<8}: {n_p:>8,} params | dropout={cfg['dropout']} "
    ...
    meta = {
        ...
        "f1_macro": round(best_m["f1"], 4),
        "accuracy": round(best_m["acc"], 4),
        "auc_macro": round(best_m["auc"], 4),
        "epochs": EPOCHS,
        "num_classes": NUM_CLASSES,
    ...
    with open(
        os.path.join(MODEL_DIR, "model_best_meta.json"), "w", encoding="utf-8"
    ) as f:
